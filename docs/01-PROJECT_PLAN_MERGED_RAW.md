...
互传到目标网盘时，如果目标路径下已经存在相同文件名，必须支持可选策略，不能静默跳过，也不能固定只走一种行为。

可选策略：

```text
overwrite_existing   直接覆盖目标中的同名文件
auto_rename_new      自动重命名后另存为新文件
```

要求：

- 冲突策略作为任务配置的一部分保存。
- 冲突判断维度就是“目标路径下已存在同路径同文件名文件”，不能把跨目录同名误判成冲突。
- 新建任务时必须让用户明确看到当前冲突策略。
- 新建任务时必须允许用户显式选择：`overwrite_existing` 或 `auto_rename_new`，不能只在内部写死一种行为。
- 默认值后续可再定，但页面上必须明确展示，不允许隐藏成内部默认行为。
- provider 原生不支持覆盖时，适配层要诚实降级：
  - 能自动重命名则走自动重命名
  - 既不能覆盖也不能安全重命名时，进入待处理并明确提示原因
- 真实联调和后续测试都要覆盖“目标路径下已存在同名文件”的场景。
