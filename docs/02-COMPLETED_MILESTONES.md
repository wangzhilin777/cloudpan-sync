...
- 完成范围：
  ...
  - provider 面板现也会直接展示 `task_runtime_track` 与说明文本，可一眼区分“Guangya / aliyundrive_open / 123_open / 115_open / xunlei / pikpak / baidu_netdisk 已在 runtime_active”“189cloud 当前 runtime_blocked”“其余已接 live list/metadata/create_dir 的 provider 属于 runtime_candidate”
...
- 当前验证证据：
  ...
  - [verify_provider_conflict_capabilities.py](E:/Workspace/VSCode/CloudPan%20Sync/scripts/verify_provider_conflict_capabilities.py) 当前还可验证状态矩阵 summary 已推进到 `taskRuntimeActiveCount=7`、`taskRuntimeCandidateCount=2`、`taskRuntimeBlockedCount=1`
