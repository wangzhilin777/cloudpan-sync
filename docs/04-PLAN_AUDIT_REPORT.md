...
- 生成时间：`2026-05-24T10:53:01.849584+00:00`
...
- 缺口：189Cloud 目前仍缺真实在线成功样本；账号级写鉴权虽已补到 captured headers/curl 提取与回填脚本，但稳定可复用的真实来源样本仍缺，shareCode/accessCode-only 档案依旧不可写。Quark/UC 的 upload 链路还未接入；百度与 PikPak 现已补到 runtime fast-upload 候选探针证据，但真实秒传 API 成功样本仍缺。
