...
### P-REAL - 真实联调验证
- 状态：`todo`
- 证据：已具备本地 mock 验证链路，并新增真实证据状态报告与任务运行真实样本持久化能力，可按 provider 量化当前已保存的 auth/list/metadata/create_dir/task_runtime 真实证据覆盖；其中 Guangya 已接真实上传链路，aliyundrive_open、123_open 与 baidu_netdisk 现也已接任务运行阶段真实本地文件上传链路，quark 与 uc 现也已接任务运行阶段真实 rapid-upload API 尝试，115_open、xunlei、pikpak 已接任务运行阶段真实 create_dir 写探针，189cloud 也已能在 share-only 场景落出 blocked probe 样本，并在账号级鉴权齐备时发起 createFolder.action 写目录尝试。
