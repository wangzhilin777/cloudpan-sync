...
- 生成时间：`2026-05-24T02:58:00.969321+00:00`
- 汇总： `providerCount=10` `profilesSaved=2` `latestValidationProfileCount=2` `latestProbeProfileCount=3`
- 真实证据覆盖： `auth=0` `list=0` `metadata=0` `create_dir=0` `fully_verified=0` `task_runtime=0` `task_runtime_failed=0` `runtime_samples=0` `runtime_success=0` `runtime_failed=0`
...
- taskRuntimeEvidence: `False` samples=0 success=0 failed=0 note=当前尚未记录到任务运行阶段真实成功样本，因此此项仍按未完成处理。
