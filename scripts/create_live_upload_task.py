...
def _remediation_followup(profile_id: str) -> dict[str, object]:
    ...
    for item in payload.get("items", []):
        ...
        return {
            ...
            "liveRejectedProfiles": list(row.get("liveRejectedProfiles") or []),
            "placeholderLiveRejectedProfiles": list(row.get("placeholderLiveRejectedProfiles") or []),
            "liveRejectedStatuses": list(row.get("liveRejectedStatuses") or []),
            "liveRejectedSummaries": list(row.get("liveRejectedSummaries") or []),
...
def _extract_live_upload_defaults(command: str) -> dict[str, object]:
    text = str(command or "").strip()
    if not text or "create_live_upload_task.py" not in text:
        return {}
    ...
    while index < len(tokens):
        ...
        if token == "--acknowledge-download-upload":
            defaults["acknowledgeDownloadUpload"] = True
        if token == "--no-acknowledge-download-upload":
            defaults["noAcknowledgeDownloadUpload"] = True
...
def main(argv: list[str] | None = None) -> int:
    ...
    target_provider = str(args.target_provider or defaults.get("targetProvider") or "guangya").strip()
    ...
    source_provider = str(args.source_provider or defaults.get("sourceProvider") or "guangya").strip()
    ...
    entry = _build_entry(file_path, str(args.remote_path or defaults.get("remotePath") or "/cloudpan-sync-live-upload.bin"))
    acknowledge_download_upload = True
    if bool(args.no_acknowledge_download_upload or defaults.get("noAcknowledgeDownloadUpload")):
        acknowledge_download_upload = False
    elif bool(args.acknowledge_download_upload or defaults.get("acknowledgeDownloadUpload")):
        acknowledge_download_upload = True
