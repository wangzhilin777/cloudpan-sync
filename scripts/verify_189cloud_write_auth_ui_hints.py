...
from cloudpan_sync.provider_auth_hints import capture_field_hints
...
def main() -> None:
    ...
    capture_hints = capture_field_hints("189cloud")
        ...
        json.dumps(
            {
                ...
                "tianyiWriteAuthUiHintsFlowMatchesExpectedFields": (
                    all(
                        token in index_html
                        for token in ("authExtraAccessToken", "authExtraSignature", "authExtraDate")
                    )
                    ...
                    and "patch_189cloud_account_auth.py" in app_js
                    and "captured_189_headers.txt" in app_js
                    and any("accessToken" in item or "token" in item for item in capture_hints)
                    and any("signature" in item for item in capture_hints)
                    and any("date" in item for item in capture_hints)
                    and "patch_189cloud_account_auth.py" in patch_command
                    and "captured_189_headers.txt" in patch_command
                ),
            },
            ensure_ascii=False,
            indent=2,
