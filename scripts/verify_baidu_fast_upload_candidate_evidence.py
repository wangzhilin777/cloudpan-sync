    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            real_evidence_report.list_profiles = lambda: [
                SimpleNamespace(profileId="baidu-fast-1", displayName="Baidu Fast Candidate", providerKey="baidu_netdisk")
            ]
        ...
        json.dumps(
            {
                ...
                "baiduFastUploadCandidateEvidenceFlowMatchesExpectedProbeOnlyReport": (
                    len(rows) == 1
                    and (results[0] if results else {}).get("executionMode") == "probe"
                    and ((results[0] if results else {}).get("liveAttempt") or {}).get("mode")
                    == "baidu_netdisk_fast_upload_candidate"
                    and ((results[0] if results else {}).get("liveAttempt") or {}).get("hashKind") == "md5"
                    and (rows[0] if rows else {}).get("providerKey") == "baidu_netdisk"
                    and (rows[0] if rows else {}).get("executionMode") == "probe"
                    and (rows[0] if rows else {}).get("mode") == "baidu_netdisk_fast_upload_candidate"
                    ...
                    and ((provider_row.get("taskRuntimeEvidence") or {}).get("candidateProfiles")) == ["Baidu Fast Candidate"]
                    ...
                    and (report.get("summary") or {}).get("taskRuntimeCandidateProviders") == ["baidu_netdisk"]
                ),
            },
            ensure_ascii=False,
            indent=2,
