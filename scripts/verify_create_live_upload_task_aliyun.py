...
def main() -> None:
    ...
    evidence_dir = ROOT / "tmp" / "verify-aliyun-live-evidence-bundle"
    ...
    def fake_get_profile(profile_id: str) -> AuthProfile | None:
        if profile_id != "ali-live-1":
            return None
        return AuthProfile(
            profileId="ali-live-1",
            providerKey="aliyundrive_open",
            authMode="official_oauth",
            displayName="ali-live",
            token="demo-token",
            cookie="",
            extra={"domainId": "domain-1", "driveId": "drive-1", "parentFileId": "root"},
            status="saved",
            lastError="",
    ...
    live_upload_script.task_runtime.upload_aliyun_open_local_file = fake_upload
    ...
    try:
        ...
        live_upload_script.task_runtime.upload_aliyun_open_local_file = original_upload
        ...
        json.dumps(
            {
                ...
                "authEvidenceRefreshed": len(refresh_calls) == 1 and refresh_calls[0].get("profileId") == "ali-live-1",
                ...
                "markdownHasConflictAction": "conflictAction=`auto_rename_new`" in markdown,
