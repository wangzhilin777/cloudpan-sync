...
def main() -> None:
    ...
    created_plan = created_item.get("plan") or {}
    print(
        json.dumps(
            {
                "createdTaskConflictPolicy": created_item.get("conflictPolicy"),
                "createdPlanConflictPolicy": created_plan.get("conflictPolicy"),
                "rowConflictPolicy": row.get("conflictPolicy"),
                "liveConflictPolicy": live_attempt.get("conflictPolicy"),
                "conflictAction": live_attempt.get("conflictAction"),
                "resolvedTargetName": live_attempt.get("resolvedTargetName"),
                "conflictPolicyFlowsThroughApiRun": (
                    created_item.get("conflictPolicy") == "overwrite_existing"
                    and created_plan.get("conflictPolicy") == "overwrite_existing"
