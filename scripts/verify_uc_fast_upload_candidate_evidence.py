...
def main() -> None:
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            real_evidence_report.list_profiles = lambda: [
                SimpleNamespace(profileId="uc-fast-1", displayName="UC Fast Candidate", providerKey="uc")
            ]
            ...
            real_evidence_report.build_provider_registry = lambda: [
                SimpleNamespace(profile=SimpleNamespace(providerKey="uc", displayName="UC Drive"))
            ]
            real_evidence_report.build_provider_research_index = lambda: [
                {"providerKey": "uc", "displayName": "UC Drive", "notes": "UC note"}
            ]
    ...
    provider_row = next((item for item in report.get("items", []) if item.get("providerKey") == "uc"), {})
    print(
        json.dumps(
            {
                ...
                "ucFastUploadCandidateEvidenceFlowMatchesExpectedProbeOnlyReport": (
                    len(rows) == 1
                    and (results[0] if results else {}).get("executionMode") == "probe"
                    and ((results[0] if results else {}).get("liveAttempt") or {}).get("mode") == "uc_fast_upload_candidate"
                    and ((results[0] if results else {}).get("liveAttempt") or {}).get("hashKind") == "md5"
                    and (rows[0] if rows else {}).get("providerKey") == "uc"
                    and (rows[0] if rows else {}).get("executionMode") == "probe"
                    and (rows[0] if rows else {}).get("mode") == "uc_fast_upload_candidate"
                    ...
                    and ((provider_row.get("taskRuntimeEvidence") or {}).get("candidateProfiles")) == ["UC Fast Candidate"]
                    ...
                    and (report.get("summary") or {}).get("taskRuntimeCandidateProviders") == ["uc"]
                ),
            },
            ensure_ascii=False,
            indent=2,
