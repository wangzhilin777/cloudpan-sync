...
def main() -> None:
    original_load_profile = uc_fast_upload_live._load_profile_requirements
    original_request_json = uc_fast_upload_live._request_json
    original_fetch_list = uc_fast_upload_live.fetch_uc_live_list

    file_path = ROOT / "tmp" / "verify-uc-conflict.bin"
    file_path.parent.mkdir(parents=True, exist_ok=True)
    file_path.write_bytes(b"uc-conflict")

    request_calls: list[tuple[str, dict[str, object]]] = []

    def fake_load_profile(profile_id: str):
        profile = type("Profile", (), {"profileId": profile_id, "providerKey": "uc", "extra": {}})()
    ...
    def fake_request_json(url: str, method: str, headers: dict[str, str], body: dict[str, object] | None = None):
        ...
        if "upload/pre" in url:
            return 200, {"code": 0, "data": {"task_id": "task-1", "obj_key": "obj/demo.bin", "bucket": "bucket-uc", "upload_id": "upload-1", "upload_url": "https://bucket-uc.example.invalid/obj/demo.bin", "auth_info": {"token": "x"}, "callback": {"callbackUrl": "https://callback.invalid"}}}
    ...
    uc_fast_upload_live._load_profile_requirements = fake_load_profile
    uc_fast_upload_live.fetch_uc_live_list = fake_fetch_list
    uc_fast_upload_live._request_json = fake_request_json
    try:
        result = uc_fast_upload_live.upload_uc_fast_file(
            profile_id="uc-profile-1",
        ...
        uc_fast_upload_live._load_profile_requirements = original_load_profile
        uc_fast_upload_live.fetch_uc_live_list = original_fetch_list
