...
def main() -> None:
        ...
        try:
            ...
            created = client.post(
                ...
                json={
                    ...
                    "targetProvider": "xunlei",
                    "targetProfileId": "xunlei-fast-1",
                    "targetParentId": "",
                    "thresholdMB": 0,
            ...
            real_evidence_report.list_profiles = lambda: [
                SimpleNamespace(profileId="xunlei-fast-1", displayName="Xunlei Fast Candidate", providerKey="xunlei")
            ]
            ...
            real_evidence_report.build_provider_registry = lambda: [
                SimpleNamespace(profile=SimpleNamespace(providerKey="xunlei", displayName="Xunlei Drive"))
            ]
            real_evidence_report.build_provider_research_index = lambda: [
                {"providerKey": "xunlei", "displayName": "Xunlei Drive", "notes": "Xunlei note"}
            ]
    ...
    provider_row = next((item for item in report.get("items", []) if item.get("providerKey") == "xunlei"), {})
    print(
        json.dumps(
            {
                ...
                "xunleiFastUploadCandidateEvidenceFlowMatchesExpectedProbeOnlyReport": (
                    len(rows) == 1
                    and (results[0] if results else {}).get("executionMode") == "probe"
                    and ((results[0] if results else {}).get("liveAttempt") or {}).get("mode") == "xunlei_fast_upload_candidate"
                    and ((results[0] if results else {}).get("liveAttempt") or {}).get("hashKind") == "gcid"
                    and (rows[0] if rows else {}).get("providerKey") == "xunlei"
                    and (rows[0] if rows else {}).get("executionMode") == "probe"
                    and (rows[0] if rows else {}).get("mode") == "xunlei_fast_upload_candidate"
                    ...
                    and ((provider_row.get("taskRuntimeEvidence") or {}).get("candidateProfiles")) == ["Xunlei Fast Candidate"]
                    ...
                    and (report.get("summary") or {}).get("taskRuntimeCandidateProviders") == ["xunlei"]
                ),
            },
            ensure_ascii=False,
            indent=2,
