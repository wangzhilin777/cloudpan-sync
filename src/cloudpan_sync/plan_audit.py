...
def run_plan_audit() -> dict[str, object]:
    ...
    items = [
        ...
        AuditItem(
            ...
            gaps="当前除 Guangya 与 aliyundrive_open 外，123_open、115_open、baidu_netdisk、xunlei、pikpak、quark、uc 仍缺真实任务运行写链路与在线成功样本验证；189Cloud 当前仍停留在 shareCode/accessCode 只读链路，尚未接入账号级 OAuth 写接口；Quark/UC 的 upload 链路还未接入，百度与 PikPak 的秒传证据也还缺失。",
        ),
        AuditItem(
            key="M6",
            title="互传任务规划",
        ...
        AuditItem(
            ...
            title="真实联调验证",
            status="todo",
            evidence="已具备本地 mock 验证链路，并新增真实证据状态报告与任务运行真实样本持久化能力，可按 provider 量化当前已保存的 auth/list/metadata/create_dir/task_runtime 真实证据覆盖；其中 Guangya 已接真实上传链路，aliyundrive_open 已接任务运行阶段真实 create_dir 写探针。",
