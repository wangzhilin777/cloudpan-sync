...
def run_plan_audit() -> dict[str, object]:
    ...
    items = [
        ...
        AuditItem(
            ...
            gaps="189Cloud 目前仍缺真实在线成功样本；账号级写鉴权虽已补到 captured headers/curl 提取与回填脚本，且现在也已补到 runtime fast-upload 候选探针证据，但稳定可复用的真实来源样本仍缺，shareCode/accessCode-only 档案依旧不可写。115 Open、Xunlei、UC 的真实 upload 链路还未接入；Quark 虽已补上真实 rapid-upload API 尝试，但普通本地文件上传链路仍未接入；PikPak、Aliyun Drive Open、123Pan Open、Baidu Netdisk、115 Open、Xunlei、Quark、UC、189Cloud 的真实秒传 API 成功样本仍缺。",
        ),
        AuditItem(
            key="M6",
            title="互传任务规划",
