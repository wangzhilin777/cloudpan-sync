...
def run_plan_audit() -> dict[str, object]:
    ...
    guangya_runtime_success_profiles = [str(value or "") for value in list(guangya_runtime.get("profiles") or []) if str(value or "")]
    guangya_runtime_orphan_profiles = [str(value or "") for value in list(guangya_runtime.get("orphanProfiles") or []) if str(value or "")]
    total_runtime_success = int(real_summary.get("taskRuntimeSuccessCount", 0) or 0)
    total_runtime_orphan_profiles = int(real_summary.get("taskRuntimeOrphanProfileCount", 0) or 0)
    runtime_success_providers = ", ".join(real_summary.get("taskRuntimeEvidenceProviders", []) or []) or "(none)"
    runtime_orphan_providers = ", ".join(real_summary.get("taskRuntimeOrphanProviders", []) or []) or "(none)"
    ...
    items = [
        ...
        AuditItem(
            ...
            status="partial",
            evidence=(
                "光鸭预检（md5/gcid）已实现，目录、metadata、create_dir、live fast-upload inventory check，以及任务运行阶段基于 targetProfileId + localPath 的 fallback live attempt "
        ...
        AuditItem(
            ...
            status="todo",
            evidence=(
                "已具备本地 mock 验证链路，并新增真实证据状态报告与任务运行真实样本持久化能力，可按 provider 量化当前已保存的 auth/list/metadata/create_dir/task_runtime 真实证据覆盖；"
