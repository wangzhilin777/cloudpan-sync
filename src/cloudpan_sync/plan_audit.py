...
def run_plan_audit() -> dict[str, object]:
    ...
    items = [
        ...
        AuditItem(
            ...
            title="真实联调验证",
            status="todo",
            evidence="已具备本地 mock 验证链路，并新增真实证据状态报告与任务运行真实样本持久化能力，可按 provider 量化当前已保存的 auth/list/metadata/create_dir/task_runtime 真实证据覆盖；其中 Guangya 已接真实上传链路，aliyundrive_open、123_open、baidu_netdisk、xunlei、pikpak、quark 与 uc 现也已接任务运行阶段真实本地文件上传链路，115_open 现已接完整的 open/upload/init + sign_check + upload/get_token + OSS 上传链路，189cloud 则已接完整的 createUploadFile -> fileUploadUrl PUT -> getUploadFileStatus -> fileCommitUrl 上传链路；189cloud 也已能在 share-only 场景落出 blocked probe 样本，并在账号级鉴权齐备时发起 createFolder.action 写目录尝试。当前真实证据状态报告已进一步明确：仓库里有 `3` 条 runtime success 样本，但它们全部落在 `guangya / uc / pikpak` 的 `runtime_orphan` 场景，也就是存在历史成功记录、但缺少当前仓库可复验的 auth profile。",
            gaps="尚未提供首批 provider 的真实联调成功证据（认证、目录、元数据、秒传/降级路径）；任务运行阶段虽已支持成功/失败样本持久化，但当前 `guangya / uc / pikpak` 的 runtime success 样本都属于 auth profile 已脱节的 `runtime_orphan` 记录，仍缺足够的可复验真实成功样本来收敛 P-REAL。",
        ),
    ]
