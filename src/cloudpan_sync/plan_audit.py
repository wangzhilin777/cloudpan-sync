...
def run_plan_audit() -> dict[str, object]:
    ...
    items = [
        ...
        AuditItem(
            ...
            gaps="已接 live 的阿里云盘 Open、189Cloud、123_open、115_open、baidu_netdisk、xunlei、pikpak、quark、uc 仍缺真实在线样本验证；189Cloud 当前仍停留在 shareCode/accessCode 只读链路，尚未接入账号级 OAuth 写接口；Quark/UC 的 upload 链路还未接入，百度与 PikPak 的秒传证据也还缺失。",
        ),
        AuditItem(
            key="M6",
            title="互传任务规划",
