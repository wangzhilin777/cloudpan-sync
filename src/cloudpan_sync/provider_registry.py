...
def build_provider_registry() -> list[ProviderAdapter]:
    providers = [
        ...
        ProviderProfile(
            ...
            fastUploadInputs=["md5", "size"],
            fallbackModes=["download_upload"],
            conflictNotes="当前 Quark 已接入任务运行阶段的 create_dir 写探针，但真实文件上传场景下的同名文件冲突处理仍未声明为已支持。",
            status="researching",
        ),
        ProviderProfile(
        ...
        ProviderProfile(
            ...
            fastUploadInputs=["md5", "size"],
            fallbackModes=["download_upload"],
            conflictNotes="当前 123Pan Open 已接入任务运行阶段的 create_dir 写探针，但真实文件上传场景下的同名文件冲突处理仍未声明为已支持。",
            status="researching",
        ),
    ]
