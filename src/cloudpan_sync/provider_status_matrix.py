...
def _runtime_track_for_provider(provider_key: str) -> tuple[str, str]:
    ...
    if provider_key == "115_open":
        return (
            "runtime_active",
            "Current task runtime now drives a 115 Open live create_dir write probe for download_upload items, and can also drive the full upload/init plus sign_check plus upload/get_token plus OSS binary upload chain for fast_upload items when a usable local file plus sha1 is available.",
        )
