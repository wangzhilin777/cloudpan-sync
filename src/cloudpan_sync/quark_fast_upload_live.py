...
def _classify_issue(status: int, error: str) -> tuple[str, str]:
    ...
    if error == "missing_upload_session":
        return ("provider", "Quark upload/pre 未返回完整上传会话，当前无法继续走二进制上传兜底。")
    if error == "missing_part_etag":
        return ("provider", "Quark 分片上传未返回 ETag，当前无法继续 commit。")
    if error.startswith("part_upload_failed"):
        return ("provider", "Quark 分片上传被拒绝，当前二进制上传兜底未完成。")
    if error.startswith("commit_failed"):
        return ("provider", "Quark multipart commit 被拒绝，当前无法完成文件落盘。")
    ...
    *,
    ...
    try:
        ...
        if not upload_finished:
            ...
            if upload_error:
                ...
                return QuarkFastUploadResult(
                    ...
                    upload_error,
                    "Quark update/hash did not hit rapid upload, and the binary upload fallback did not complete.",
                    risk_level,
                    risk_hint,
                    payload={
                        "preUploadResponse": pre_payload,
        ...
        return QuarkFastUploadResult(
            ...
            verifyNote="Quark binary upload fallback was confirmed by the provider finish response."
            if used_binary_fallback
            else "Quark rapid-upload success was confirmed by the provider finish response.",
            verifyPayload={
                "fileId": file_id,
                "taskId": task_id,
                "objKey": obj_key,
