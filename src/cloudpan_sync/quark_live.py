...
QUARK_API_BASE = "https://pc-api.uc.cn"
QUARK_DRIVE_API_BASE = "https://drive-pc.quark.cn"
QUARK_PC_USER_AGENT = "Mozilla/5.0 QuarkCloudDrivePC/1.0"
...
def _get_share_token(pwd_id: str, passcode: str, cookie: str) -> tuple[int, dict[str, object], str]:
    status, payload = _request_json(
        f"{QUARK_API_BASE}/1/clouddrive/share/sharepage/token",
        "POST",
        _headers(cookie),
...
def _build_detail_url(pwd_id: str, stoken: str, pdir_fid: str, page: int, size: int) -> str:
    ...
    return f"{QUARK_API_BASE}/1/clouddrive/share/sharepage/detail?{urlencode(params)}"


def _build_drive_file_url() -> str:
    return f"{QUARK_DRIVE_API_BASE}/1/clouddrive/file?{urlencode({'pr': 'ucpro', 'fr': 'pc', 'uc_param_str': ''})}"
...
def _fetch_md5_map(items: list[dict[str, object]], pwd_id: str, stoken: str, cookie: str) -> tuple[int, dict[str, str], dict[str, object]]:
    ...
    status, payload = _request_json(
        f"{QUARK_API_BASE}/1/clouddrive/file/download?{urlencode({'pr': 'ucpro', 'fr': 'pc', 'uc_param_str': ''})}",
        "POST",
        _headers(cookie),
        body,
    )
...
def fetch_quark_live_list(profile_id: str, parent_id: str = "0", page_size: int = 200) -> QuarkLiveResult:
    profile, cookie, pwd_id, passcode = _load_profile_requirements(profile_id)
    if profile is None:
        return QuarkLiveResult(False, "profile_missing", False, profile_id, 0, "profile_not_found", "Saved Quark auth profile was not found.", {})
    if not cookie:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_cookie", "Quark live list requires cookie or extra.cookie_header.", {})
    if not pwd_id:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_pwd_id", "Quark live list requires extra.pwdId or extra.sharePwdId.", {})
    ...
    try:
        ...
        if token_status < 200 or token_status >= 300 or not stoken:
            return QuarkLiveResult(False, "live_error", True, profile.profileId, token_status, "share_token_failed", "Quark share token request did not return a usable stoken.", {"raw": token_payload})
        status, payload = _request_json(
        ...
        if not _is_success(payload):
            return QuarkLiveResult(False, "live_error", True, profile.profileId, status, "detail_failed", "Quark live list request was rejected.", {"raw": payload})
        items = [_normalize_item(item, parent_id or "0") for item in _extract_list(payload)]
        return QuarkLiveResult(
            True,
            "live",
            True,
            profile.profileId,
            status,
            "",
            "Quark share-based live list succeeded with saved cookie and pwdId.",
            {"items": items, "raw": payload, "pwdId": pwd_id, "parentId": parent_id or "0", "stoken": stoken, "total": _extract_total(payload)},
        )
    except HTTPError as exc:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, int(exc.code or 0), f"http_error:{exc.code}", "Quark live list reached the API but was rejected.", {})
    except URLError as exc:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, 0, f"url_error:{exc.reason}", "Quark live list could not reach the API endpoint.", {})
    except json.JSONDecodeError:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, 200, "invalid_json", "Quark live list returned non-JSON content.", {})
    except Exception as exc:  # pragma: no cover
        return QuarkLiveResult(False, "live_error", True, profile.profileId, 0, f"unexpected:{exc}", "Quark live list failed unexpectedly.", {})


def fetch_quark_live_metadata(profile_id: str, file_id: str, parent_id: str = "0") -> QuarkLiveResult:
    profile, cookie, pwd_id, passcode = _load_profile_requirements(profile_id)
    if profile is None:
        return QuarkLiveResult(False, "profile_missing", False, profile_id, 0, "profile_not_found", "Saved Quark auth profile was not found.", {})
    if not cookie:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_cookie", "Quark live metadata requires cookie or extra.cookie_header.", {})
    if not pwd_id:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_pwd_id", "Quark live metadata requires extra.pwdId or extra.sharePwdId.", {})
    resolved_file_id = _text(file_id or profile.extra.get("fileId"))
    if not resolved_file_id:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_file_id", "Quark live metadata requires fileId.", {})

    list_result = fetch_quark_live_list(profile.profileId, parent_id=parent_id or "0")
    if not list_result.ok:
        return QuarkLiveResult(
            ...
            list_result.status,
            list_result.error,
            f"Quark live metadata failed because parent page listing did not succeed: {list_result.note}",
            list_result.payload,
        )
    ...
    if matched is None:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, list_result.status, "metadata_not_found", "Quark live metadata did not find the file under the provided parentId page.", list_result.payload)
    ...
    if md5_status < 200 or md5_status >= 300:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, md5_status, "download_info_failed", "Quark metadata MD5 request was rejected.", {"raw": md5_payload})
    entry = {
    ...
    return QuarkLiveResult(
        ...
        list_result.status,
        "",
        "Quark live metadata succeeded through share detail + file/download MD5 lookup.",
        {"entry": entry, "raw": {"detail": list_result.payload.get("raw", {}), "download": md5_payload}, "parentId": parent_id or "0", "pwdId": pwd_id},
    )


def fetch_quark_create_folder(profile_id: str, parent_id: str = "0", dir_name: str = "") -> QuarkLiveResult:
    profile, cookie, _, _ = _load_profile_requirements(profile_id)
    if profile is None:
        return QuarkLiveResult(False, "profile_missing", False, profile_id, 0, "profile_not_found", "Saved Quark auth profile was not found.", {})
    if not cookie:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_cookie", "Quark create folder requires cookie or extra.cookie_header.", {})
    resolved_dir_name = _text(dir_name or (profile.extra or {}).get("dirName"))
    if not resolved_dir_name:
        return QuarkLiveResult(False, "profile_incomplete", True, profile.profileId, 0, "missing_dir_name", "Quark create folder requires dirName.", {})
    ...
    try:
        ...
        if not _is_success(payload):
            return QuarkLiveResult(False, "live_error", True, profile.profileId, status, "create_dir_failed", "Quark create folder request was rejected.", {"raw": payload})
        ...
        return QuarkLiveResult(True, "live", True, profile.profileId, status, "", "Quark live create folder succeeded with saved cookie.", {"item": item, "parentId": resolved_parent_id, "raw": payload})
    except HTTPError as exc:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, int(exc.code or 0), f"http_error:{exc.code}", "Quark create folder reached the API but was rejected.", {})
    except URLError as exc:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, 0, f"url_error:{exc.reason}", "Quark create folder could not reach the API endpoint.", {})
    except json.JSONDecodeError:
        return QuarkLiveResult(False, "live_error", True, profile.profileId, 200, "invalid_json", "Quark create folder returned non-JSON content.", {})
    except Exception as exc:  # pragma: no cover
        return QuarkLiveResult(False, "live_error", True, profile.profileId, 0, f"unexpected:{exc}", "Quark create folder failed unexpectedly.", {})
