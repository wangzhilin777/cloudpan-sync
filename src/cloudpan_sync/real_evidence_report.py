...
def _probe_runtime_profile_labels(
    rows: list[dict[str, object]],
    profile_map: dict[str, object],
) -> list[str]:
    labels: list[str] = []
    for row in rows:
        if bool(row.get("probeOnly")) and not bool(row.get("candidateOnly")):
            labels.append(_profile_label(profile_map, str(row.get("profileId") or "")))
    return sorted(set(labels))
...
def build_real_evidence_report() -> dict[str, object]:
    ...
    for provider_key in provider_keys:
        ...
        provider_runtime_effective_rows = [
            row
            for row in provider_runtime_rows
            if not bool(row.get("candidateOnly")) and not bool(row.get("probeOnly"))
        ]
        provider_runtime_candidate_rows = [row for row in provider_runtime_rows if bool(row.get("candidateOnly"))]
        provider_runtime_probe_rows = [
            row
            for row in provider_runtime_rows
            if bool(row.get("probeOnly")) and not bool(row.get("candidateOnly"))
        ]
        ...
        runtime_probe = bool(runtime_probe_labels)
        ...
        if runtime_probe and not runtime_ok:
        ...
        items.append(
            {
                ...
                "taskRuntimeEvidence": {
                    ...
                    "probeCount": len(provider_runtime_probe_rows),
                    ...
                    "note": (
                        ...
                        if runtime_ok
                            ...
                            if runtime_failed
                                ...
                                "当前仅记录到 probe-only 样本，尚未出现真实传输成功样本。"
                                if runtime_probe
                                else (
    ...
    return {
        ...
        "summary": {
            ...
            "taskRuntimeProbeProviderCount": sum(1 for item in items if int(((item.get("taskRuntimeEvidence") or {}).get("probeCount", 0)) or 0) > 0),
            "taskRuntimeSampleCount": len(runtime_rows),
            "taskRuntimeSuccessCount": sum(
                1
                for row in runtime_rows
                if bool(row.get("success")) and not bool(row.get("candidateOnly")) and not bool(row.get("probeOnly"))
            ),
            "taskRuntimeFailedCount": sum(
                1
                for row in runtime_rows
                if not bool(row.get("success")) and not bool(row.get("candidateOnly")) and not bool(row.get("probeOnly"))
            ),
            "taskRuntimeCandidateCount": sum(1 for row in runtime_rows if bool(row.get("candidateOnly"))),
            "taskRuntimeProbeCount": sum(
                1
                for row in runtime_rows
                if bool(row.get("probeOnly")) and not bool(row.get("candidateOnly"))
            ),
...
def real_evidence_to_markdown(payload: dict[str, object]) -> str:
    ...
    lines.append(
        ...
        f" `task_runtime_probe={summary.get('taskRuntimeProbeProviderCount', 0)}`"
        ...
        f" `runtime_probe={summary.get('taskRuntimeProbeCount', 0)}`"
