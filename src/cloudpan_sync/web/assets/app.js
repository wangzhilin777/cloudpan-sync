...
function renderSettingsPanel() {
  ...
  const remediationRows = [
    `providers=${remediationSummary.providerCount || 0}, noProfiles=${remediationSummary.providersWithNoProfiles || 0}, createCommands=${remediationSummary.providersWithCreateCommand || 0}, patchCommands=${remediationSummary.providersWithPatchCommand || 0}, blockedOnly=${remediationSummary.providersBlockedOnly || 0}`,
  ...
  for (const item of remediationItems.filter((row) => row.nextStep).slice(0, 3)) {
    ...
    li.textContent = `${item.providerKey || "(unknown)"}: profiles=${item.profileCount || 0}, authModes=${authModes}, nextStep=${item.nextStep}${loginUrl ? `, login=${loginUrl}` : ""}${fieldHints ? `, hints=${fieldHints}` : ""}${item.recommendedCreateCommand ? `, create=${item.recommendedCreateCommand}` : ""}${item.recommendedPatchCommand ? `, patch=${item.recommendedPatchCommand}` : ""}`;
    realEvidenceRemediationList.appendChild(li);
  }
