  ...
  const hasOrphanFollowup = Boolean(
    followupProfileId
      && (
        ...
        || data?.exactCreateHelper
        || data?.recommendedRefreshEvidenceCommand
        || data?.exactRefreshEvidenceHelper
        || data?.recommendedRuntimeProbeCommand
        || data?.exactRuntimeProbeHelper
        || data?.recommendedRuntimeSuccessCommand
        || data?.exactRuntimeSuccessHelper
        || data?.exactPostRefreshRuntimeHelper
        || data?.recommendedPostBootstrapRuntimeCommand
        || data?.exactPostBootstrapRuntimeHelper
        || data?.recommendedOverwriteVariantCommand
        || data?.exactOverwriteVariantHelper
      )
  );
...
function renderSettingsPanel() {
  ...
  if (firstAuthRemediationGap) {
    ...
    const firstLiveRejectedSummaries = (firstAuthRemediationGap.liveRejectedSummaries || []).join(" | ") || "";
  ...
  const firstRealEvidenceOrphanItem = (state.runtimeOrphanRecovery?.items || [])[0] || null;
  if ((realEvidence.taskRuntimeOrphanProfileCount || 0) > 0) {
    const li = document.createElement("li");
    const copy = document.createElement("span");
    copy.textContent = `runtime_orphan_recovery: providers=${realEvidence.taskRuntimeOrphanProviderCount || 0}, profiles=${realEvidence.taskRuntimeOrphanProfileCount || 0}, firstProvider=${firstRealEvidenceOrphanItem?.providerKey || "(none)"}, firstProfile=${firstRealEvidenceOrphanItem?.orphanProfileId || "(none)"}`;
    li.appendChild(copy);
    const actions = document.createElement("span");
    actions.className = "row-actions";
    const openOrphanBtn = document.createElement("button");
  ...
  if (firstRealEvidenceGap) {
    ...
    const firstGapLabels = {
      ...
      probe: firstGapHasProfile ? "Probe Existing Profile" : "Run First Probe",
      capture: firstGapHasProfile ? "Open Capture For Existing Profile" : "Open Capture First Gap",
    };
    const li = document.createElement("li");
    const copy = document.createElement("span");
    copy.textContent = `${firstRealEvidenceGap.providerKey || "(unknown)"}: nextStep=${firstRealEvidenceGap.nextStep || "(none)"}, needsRuntime=${Boolean(firstRealEvidenceGap.needsRuntimeSuccess)}, needsAuth=${Boolean(firstRealEvidenceGap.needsAuthEvidence)}, runtimeOrphanOnly=${Boolean(firstRealEvidenceGap.runtimeOrphanOnly)}, candidateOnly=${Boolean(firstRealEvidenceGap.runtimeCandidateOnly)}, probeOnly=${Boolean(firstRealEvidenceGap.runtimeProbeOnly)}`;
    li.appendChild(copy);
    const actions = document.createElement("span");
    actions.className = "row-actions";
    const profileId = (firstRealEvidenceGap.profileIds || [])[0] || "";
    if (profileId) {
      const focusBtn = document.createElement("button");
      focusBtn.className = "ghost";
      focusBtn.textContent = firstGapLabels.focus;
    ...
    const captureBtn = document.createElement("button");
    captureBtn.className = "ghost";
    captureBtn.textContent = firstGapLabels.capture;
    captureBtn.addEventListener("click", () => openCaptureGuideForProvider(firstRealEvidenceGap.providerKey || ""));
    actions.appendChild(captureBtn);
    if (firstRealEvidenceGap.recommendedCreateCommand) {
      const createBtn = document.createElement("button");
      createBtn.className = "ghost";
      createBtn.textContent = "Create Stub First Gap";
  ...
  if (lastRemediationAction?.status) {
    ...
    copy.textContent = `latestRemediationAction=${lastRemediationAction.status || "(unknown)"}, provider=${latestProfile?.providerKey || "(unknown)"}, profileId=${latestProfile?.profileId || "(none)"}, nextStep=${lastRemediationAction.nextStep || "(none)"}${lastRemediationAction.exactCreateHelper ? `, exactCreate=${lastRemediationAction.exactCreateHelper}` : ""}${lastRemediationAction.exactRecreateHelper ? `, exactRecreate=${lastRemediationAction.exactRecreateHelper}` : ""}${lastRemediationAction.recommendedRefreshEvidenceCommand ? `, refresh=${lastRemediationAction.recommendedRefreshEvidenceCommand}` : ""}${lastRemediationAction.exactRefreshEvidenceHelper ? `, exactRefresh=${lastRemediationAction.exactRefreshEvidenceHelper}` : ""}${lastRemediationAction.recommendedPostRefreshRuntimeCommand ? `, postRefreshRuntime=${lastRemediationAction.recommendedPostRefreshRuntimeCommand}` : ""}${lastRemediationAction.exactPostRefreshRuntimeHelper ? `, exactPostRefreshRuntime=${lastRemediationAction.exactPostRefreshRuntimeHelper}` : ""}${lastRemediationAction.recommendedRuntimeProbeCommand ? `, runtimeProbe=${lastRemediationAction.recommendedRuntimeProbeCommand}` : ""}${lastRemediationAction.exactRuntimeProbeHelper ? `, exactRuntimeProbe=${lastRemediationAction.exactRuntimeProbeHelper}` : ""}${lastRemediationAction.recommendedRuntimeSuccessCommand ? `, runtimeSuccess=${lastRemediationAction.recommendedRuntimeSuccessCommand}` : ""}${lastRemediationAction.exactRuntimeSuccessHelper ? `, exactRuntimeSuccess=${lastRemediationAction.exactRuntimeSuccessHelper}` : ""}${lastRemediationAction.recommendedPostBootstrapRuntimeCommand ? `, postBootstrapRuntime=${lastRemediationAction.recommendedPostBootstrapRuntimeCommand}` : ""}${lastRemediationAction.exactPostBootstrapRuntimeHelper ? `, exactPostBootstrapRuntime=${lastRemediationAction.exactPostBootstrapRuntimeHelper}` : ""}${lastRemediationAction.recommendedOverwriteVariantCommand ? `, overwriteVariant=${lastRemediationAction.recommendedOverwriteVariantCommand}` : ""}${lastRemediationAction.exactOverwriteVariantHelper ? `, exactOverwriteVariant=${lastRemediationAction.exactOverwriteVariantHelper}` : ""}`;
    li.appendChild(copy);
  ...
  const remediationRows = [
    ...
    `blockedOnlyProviders=${(remediationSummary.providersBlockedOnlyList || []).join("/") || "(none)"}, candidateOnlyProviders=${(remediationSummary.providersCandidateOnlyList || []).join("/") || "(none)"}, probeOnlyProviders=${(remediationSummary.providersProbeOnlyList || []).join("/") || "(none)"}, runtimeOrphanOnlyProviders=${(remediationSummary.providersRuntimeOrphanOnlyList || []).join("/") || "(none)"}`,
  ];
  ...
  for (const item of remediationItems.filter((row) => row.nextStep).slice(0, 3)) {
    ...
    const copy = document.createElement("span");
    copy.textContent = `${item.providerKey || "(unknown)"}: profiles=${item.profileCount || 0}, authModes=${authModes}, nextStep=${item.nextStep}, blockedOnly=${Boolean(item.runtimeBlockedOnly)}, candidateOnly=${Boolean(item.runtimeCandidateOnly)}, probeOnly=${Boolean(item.runtimeProbeOnly)}, runtimeOrphanOnly=${Boolean(item.runtimeOrphanOnly)}, needsSecretRefresh=${Boolean(item.needsSecretRefresh)}, conflictDeclared=${(item.declaredConflictPolicies || []).join("/") || "(none)"}, overwriteSupport=${item.overwriteSupportStatus || "unknown"}, autoRenameSupport=${item.autoRenameSupportStatus || "unknown"}, overwriteBehavior=${item.overwriteBehavior || "unknown"}${loginUrl ? `, login=${loginUrl}` : ""}${fieldHints ? `, hints=${fieldHints}` : ""}${placeholderSecretHints ? `, placeholderSecretHints=${placeholderSecretHints}` : ""}${liveRejectedStatuses ? `, liveRejectedStatuses=${liveRejectedStatuses}` : ""}${placeholderLiveRejectedProfiles ? `, placeholderLiveRejectedProfiles=${placeholderLiveRejectedProfiles}` : ""}${liveRejectedSummaries ? `, liveRejectedSummaries=${liveRejectedSummaries}` : ""}${(item.runtimeOrphanProfiles || []).length ? `, runtimeOrphanProfiles=${(item.runtimeOrphanProfiles || []).join("/")}` : ""}${item.providerConflictNotes ? `, providerConflictNotes=${item.providerConflictNotes}` : ""}${item.recommendedPrimaryCommand ? `, primary=${item.recommendedPrimaryCommand}` : ""}${item.recommendedPrimaryCommandLabel ? `, primaryLabel=${item.recommendedPrimaryCommandLabel}` : ""}${item.recommendedCreateCommand ? `, create=${item.recommendedCreateCommand}` : ""}${item.exactCreateHelper ? `, exactCreate=${item.exactCreateHelper}` : ""}${item.recommendedBootstrapCommand ? `, bootstrap=${item.recommendedBootstrapCommand}` : ""}${item.recommendedPatchCommand ? `, patch=${item.recommendedPatchCommand}` : ""}${item.recommendedPatchProbeCommand ? `, patchProbe=${item.recommendedPatchProbeCommand}` : ""}${item.exactPatchHelper ? `, exactPatch=${item.exactPatchHelper}` : ""}${item.recommendedRecreateProbeCommand ? `, recreateProbe=${item.recommendedRecreateProbeCommand}` : ""}${item.exactRecreateHelper ? `, exactRecreate=${item.exactRecreateHelper}` : ""}${item.recommendedRefreshEvidenceCommand ? `, refresh=${item.recommendedRefreshEvidenceCommand}` : ""}${item.exactRefreshEvidenceHelper ? `, exactRefresh=${item.exactRefreshEvidenceHelper}` : ""}${item.recommendedPostRefreshRuntimeCommand ? `, postRefreshRuntime=${item.recommendedPostRefreshRuntimeCommand}` : ""}${item.exactPostRefreshRuntimeHelper ? `, exactPostRefreshRuntime=${item.exactPostRefreshRuntimeHelper}` : ""}${item.recommendedRuntimeProbeCommand ? `, runtime=${item.recommendedRuntimeProbeCommand}` : ""}${item.exactRuntimeProbeHelper ? `, exactRuntime=${item.exactRuntimeProbeHelper}` : ""}${item.recommendedLiveUploadCommand ? `, liveUpload=${item.recommendedLiveUploadCommand}` : ""}${item.recommendedFastCandidateCommand ? `, fastCandidate=${item.recommendedFastCandidateCommand}` : ""}${item.recommendedRuntimeSuccessCommand ? `, runtimeSuccess=${item.recommendedRuntimeSuccessCommand}` : ""}${item.exactRuntimeSuccessHelper ? `, exactRuntimeSuccess=${item.exactRuntimeSuccessHelper}` : ""}${item.recommendedPostBootstrapRuntimeCommand ? `, postBootstrapRuntime=${item.recommendedPostBootstrapRuntimeCommand}` : ""}${item.exactPostBootstrapRuntimeHelper ? `, exactPostBootstrapRuntime=${item.exactPostBootstrapRuntimeHelper}` : ""}${item.recommendedOverwriteVariantCommand ? `, overwriteVariant=${item.recommendedOverwriteVariantCommand}` : ""}${item.exactOverwriteVariantHelper ? `, exactOverwriteVariant=${item.exactOverwriteVariantHelper}` : ""}${item.conflictPolicyNote ? `, conflictPolicyNote=${item.conflictPolicyNote}` : ""}`;
    li.appendChild(copy);
  ...
  const firstRuntimeOrphanItem = runtimeOrphanRows[0] || null;
  if (runtimeOrphanRows.length > 0) {
    const li = document.createElement("li");
    const copy = document.createElement("span");
    copy.textContent = `runtime_orphan_recovery: providers=${runtimeOrphanProviders.length || 0}, profiles=${runtimeOrphanProfiles.length || 0}, firstProvider=${firstRuntimeOrphanItem?.providerKey || "(none)"}, firstProfile=${firstRuntimeOrphanItem?.profileId || "(none)"}`;
    li.appendChild(copy);
    const actions = document.createElement("span");
    actions.className = "row-actions";
    const openOrphanBtn = document.createElement("button");
  ...
  if (firstRuntimeEvidenceGap) {
    ...
    const firstRuntimeLabels = {
      ...
      probe: hasExistingProfile ? "Probe Existing Profile" : "Probe First Runtime",
      capture: hasExistingProfile ? "Open Capture For Existing Profile" : "Open Capture First Runtime",
    };
    const li = document.createElement("li");
    const copy = document.createElement("span");
    ...
    if (matchedProfile) {
      const focusBtn = document.createElement("button");
      focusBtn.className = "ghost";
      focusBtn.textContent = firstRuntimeLabels.focus;
      focusBtn.addEventListener("click", () => focusAuthRemediationProfile(matchedProfile.profileId));
      actions.appendChild(focusBtn);
      const refreshBtn = document.createElement("button");
      refreshBtn.className = "ghost";
      refreshBtn.textContent = firstRuntimeLabels.refresh;
      refreshBtn.addEventListener("click", () => refreshRealEvidenceRemediationProfile(matchedProfile.profileId));
      actions.appendChild(refreshBtn);
      if (liveProbeProviderSet.has(matchedProfile.providerKey)) {
        const probeBtn = document.createElement("button");
        probeBtn.className = "ghost";
        probeBtn.textContent = firstRuntimeLabels.probe;
        probeBtn.addEventListener("click", () => probeRealEvidenceRemediationProfile(matchedProfile.profileId));
        actions.appendChild(probeBtn);
      }
    }
    const captureBtn = document.createElement("button");
    captureBtn.className = "ghost";
    captureBtn.textContent = firstRuntimeLabels.capture;
  ...
  if (lastRuntimeOrphanAction?.status) {
    ...
    copy.textContent = `latestRuntimeOrphanAction=${lastRuntimeOrphanAction.status || "(unknown)"}, provider=${latestProfile?.providerKey || "(unknown)"}, profileId=${latestProfile?.profileId || "(none)"}, nextStep=${lastRuntimeOrphanAction.nextStep || "(none)"}${lastRuntimeOrphanAction.recommendedPrimaryCommand ? `, primary=${lastRuntimeOrphanAction.recommendedPrimaryCommand}` : ""}${lastRuntimeOrphanAction.recommendedPrimaryCommandLabel ? `, primaryLabel=${lastRuntimeOrphanAction.recommendedPrimaryCommandLabel}` : ""}${lastRuntimeOrphanAction.exactCreateHelper ? `, exactRecreate=${lastRuntimeOrphanAction.exactCreateHelper}` : ""}${lastRuntimeOrphanAction.recommendedRefreshEvidenceCommand ? `, refresh=${lastRuntimeOrphanAction.recommendedRefreshEvidenceCommand}` : ""}${lastRuntimeOrphanAction.exactRefreshEvidenceHelper ? `, exactRefresh=${lastRuntimeOrphanAction.exactRefreshEvidenceHelper}` : ""}${lastRuntimeOrphanAction.recommendedRuntimeProbeCommand ? `, runtimeProbe=${lastRuntimeOrphanAction.recommendedRuntimeProbeCommand}` : ""}${lastRuntimeOrphanAction.exactRuntimeProbeHelper ? `, exactRuntimeProbe=${lastRuntimeOrphanAction.exactRuntimeProbeHelper}` : ""}${lastRuntimeOrphanAction.recommendedRuntimeSuccessCommand ? `, runtimeSuccess=${lastRuntimeOrphanAction.recommendedRuntimeSuccessCommand}` : ""}${lastRuntimeOrphanAction.exactRuntimeSuccessHelper ? `, exactRuntimeSuccess=${lastRuntimeOrphanAction.exactRuntimeSuccessHelper}` : ""}${lastRuntimeOrphanAction.recommendedOverwriteVariantCommand ? `, overwriteVariant=${lastRuntimeOrphanAction.recommendedOverwriteVariantCommand}` : ""}${lastRuntimeOrphanAction.exactOverwriteVariantHelper ? `, exactOverwriteVariant=${lastRuntimeOrphanAction.exactOverwriteVariantHelper}` : ""}`;
    li.appendChild(copy);
    if (latestProfile?.profileId || latestProfile?.providerKey) {
      const actions = document.createElement("span");
      actions.className = "row-actions";
  ...
  for (const item of orphanRecoveryItems.slice(0, 3)) {
    ...
    const orphanRowLabels = existingProfileId ? {
      focus: "Focus Existing Orphan Profile",
      refresh: "Refresh Existing Orphan Profile",
      probe: "Probe Existing Orphan Profile",
      capture: "Open Capture For Existing Orphan Profile",
    } : null;
    const copy = document.createElement("span");
    copy.textContent = `${item.providerKey || "(unknown)"}: orphanProfileId=${item.orphanProfileId || "(none)"}, sampleCount=${item.sampleCount || 0}, authModes=${authModes}, preferredAuthMode=${item.preferredAuthMode || "(none)"}, existingProfiles=${existingProfiles}${fieldHints ? `, hints=${fieldHints}` : ""}${item.webLoginUrl ? `, login=${item.webLoginUrl}` : ""}${item.recommendedPrimaryCommand ? `, primary=${item.recommendedPrimaryCommand}` : ""}${item.recommendedPrimaryCommandLabel ? `, primaryLabel=${item.recommendedPrimaryCommandLabel}` : ""}${item.recommendedCreateCommand ? `, recreate=${item.recommendedCreateCommand}` : ""}${item.exactCreateHelper ? `, exactRecreate=${item.exactCreateHelper}` : ""}${item.recommendedRefreshEvidenceCommand ? `, refresh=${item.recommendedRefreshEvidenceCommand}` : ""}${item.exactRefreshEvidenceHelper ? `, exactRefresh=${item.exactRefreshEvidenceHelper}` : ""}${item.recommendedRuntimeProbeCommand ? `, runtimeProbe=${item.recommendedRuntimeProbeCommand}` : ""}${item.exactRuntimeProbeHelper ? `, exactRuntimeProbe=${item.exactRuntimeProbeHelper}` : ""}${item.recommendedRuntimeSuccessCommand ? `, runtimeSuccess=${item.recommendedRuntimeSuccessCommand}` : ""}${item.exactRuntimeSuccessHelper ? `, exactRuntimeSuccess=${item.exactRuntimeSuccessHelper}` : ""}${item.recommendedOverwriteVariantCommand ? `, overwriteVariant=${item.recommendedOverwriteVariantCommand}` : ""}${item.exactOverwriteVariantHelper ? `, exactOverwriteVariant=${item.exactOverwriteVariantHelper}` : ""}`;
    li.appendChild(copy);
    const actions = document.createElement("span");
    actions.className = "row-actions";
    ...
    const captureBtn = document.createElement("button");
    captureBtn.className = "ghost";
    captureBtn.textContent = existingProfileId ? orphanRowLabels.capture : "Open Capture";
    captureBtn.addEventListener("click", () => openCaptureGuideForProvider(item.providerKey));
    actions.appendChild(captureBtn);
    li.appendChild(actions);
    runtimeOrphanRecoveryList.appendChild(li);
  ...
  if (firstRuntimeOrphanGap) {
    const hasExistingProfile = Boolean((firstRuntimeOrphanGap.existingProviderProfileIds || [])[0] || "");
    const firstGapLabels = {
      focus: hasExistingProfile ? "Focus Existing Orphan Profile" : "Focus First Match",
      refresh: hasExistingProfile ? "Refresh Existing Orphan Profile" : "Refresh First Match",
      probe: hasExistingProfile ? "Probe Existing Orphan Profile" : "Probe First Match",
      capture: hasExistingProfile ? "Open Capture For Existing Orphan Profile" : "Open Capture First Gap",
    };
    const li = document.createElement("li");
    const copy = document.createElement("span");
    const existingProfiles = (firstRuntimeOrphanGap.existingProviderProfileNames || []).join("/") || "(none)";
    copy.textContent = `${firstRuntimeOrphanGap.providerKey || "(unknown)"}: orphanProfileId=${firstRuntimeOrphanGap.orphanProfileId || "(none)"}, sampleCount=${firstRuntimeOrphanGap.sampleCount || 0}, preferredAuthMode=${firstRuntimeOrphanGap.preferredAuthMode || "(none)"}, existingProfiles=${existingProfiles}, nextStep=${firstRuntimeOrphanGap.nextStep || "(none)"}${firstRuntimeOrphanGap.recommendedPrimaryCommand ? `, primary=${firstRuntimeOrphanGap.recommendedPrimaryCommand}` : ""}${firstRuntimeOrphanGap.recommendedPrimaryCommandLabel ? `, primaryLabel=${firstRuntimeOrphanGap.recommendedPrimaryCommandLabel}` : ""}${firstRuntimeOrphanGap.exactCreateHelper ? `, exactRecreate=${firstRuntimeOrphanGap.exactCreateHelper}` : ""}${firstRuntimeOrphanGap.exactRefreshEvidenceHelper ? `, exactRefresh=${firstRuntimeOrphanGap.exactRefreshEvidenceHelper}` : ""}${firstRuntimeOrphanGap.recommendedRuntimeSuccessCommand ? `, runtimeSuccess=${firstRuntimeOrphanGap.recommendedRuntimeSuccessCommand}` : ""}${firstRuntimeOrphanGap.exactRuntimeSuccessHelper ? `, exactRuntimeSuccess=${firstRuntimeOrphanGap.exactRuntimeSuccessHelper}` : ""}${firstRuntimeOrphanGap.recommendedOverwriteVariantCommand ? `, overwriteVariant=${firstRuntimeOrphanGap.recommendedOverwriteVariantCommand}` : ""}${firstRuntimeOrphanGap.exactOverwriteVariantHelper ? `, exactOverwriteVariant=${firstRuntimeOrphanGap.exactOverwriteVariantHelper}` : ""}`;
    li.appendChild(copy);
    const actions = document.createElement("span");
    actions.className = "row-actions";
  ...
  const auditRuntimeOrphanItem = (state.runtimeOrphanRecovery?.items || [])[0] || null;
  if ((state.realEvidenceSummary?.taskRuntimeOrphanProfileCount || 0) > 0) {
    const li = document.createElement("li");
    const copy = document.createElement("span");
    copy.textContent = `runtime_orphan_recovery: providers=${state.realEvidenceSummary?.taskRuntimeOrphanProviderCount || 0}, profiles=${state.realEvidenceSummary?.taskRuntimeOrphanProfileCount || 0}, firstProvider=${auditRuntimeOrphanItem?.providerKey || "(none)"}, firstProfile=${auditRuntimeOrphanItem?.orphanProfileId || "(none)"}`;
    li.appendChild(copy);
    const actions = document.createElement("span");
    actions.className = "row-actions";
    const openOrphanBtn = document.createElement("button");
  ...
  if (firstAuditGap) {
    ...
    if (["M4", "M5", "P-REAL"].includes(firstAuditGap.key)) {
      ...
      firstAuditGapOrphanItems = firstAuditGap.key === "M4"
        ? runtimeOrphanItems.filter((item) => item.providerKey === "guangya")
        : runtimeOrphanItems;
      firstAuditGapOrphanItem = firstAuditGapOrphanItems[0] || null;
    ...
    if (["M4", "M5", "P-REAL"].includes(firstAuditGap.key)) {
      ...
      actions.appendChild(openAuthBtn);
      appendRuntimeOrphanRecreateButtons(
        actions,
        firstAuditGapOrphanItems,
        firstAuditGap.key === "M4" ? "Recreate Guangya Orphan Stub" : "Recreate Orphan Stub"
      );
    }
    li.appendChild(actions);
    auditList.appendChild(li);
