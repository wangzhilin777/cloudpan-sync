...
def create_app() -> FastAPI:
    ...
    @app.post("/api/runtime_orphan_recovery/recreate_profile")
    def runtime_orphan_recovery_recreate_profile(payload: RuntimeOrphanRecreateRequest, request: Request) -> dict[str, object]:
        if not _is_logged_in(request):
            raise HTTPException(status_code=401, detail="please_login_first")
